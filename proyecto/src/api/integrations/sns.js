...
const sendSpaceNotification = async (event) => {
  try {
    ...
    logger.error('Error sending space notification:', { errorMessage: error.message, errorType: error.constructor.name });
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ 
...
const sendSystemAlert = async (event) => {
  try {
    ...
    logger.error('Error sending system alert:', { errorMessage: error.message, errorType: error.constructor.name });
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ 
...
const sendAdminNotification = async (event) => {
  try {
    ...
    logger.error('Error sending admin notification:', { errorMessage: error.message, errorType: error.constructor.name });
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ 
...
const processSpaceNotification = async (event) => {
  try {
    ...
    for (const record of event.Records) {
      ...
      logger.info(' Space notification processed: ${message.actionType} for space ${message.spaceId}');
    }
    ...
    logger.error('Error processing space notification:', { errorMessage: error.message, errorType: error.constructor.name });
    throw error;
  }
};
...
const processSystemAlert = async (event) => {
  try {
    ...
    logger.error('Error processing system alert:', { errorMessage: error.message, errorType: error.constructor.name });
    throw error;
  }
};
...
const processAdminNotification = async (event) => {
  try {
    ...
    logger.error('Error processing admin notification:', { errorMessage: error.message, errorType: error.constructor.name });
    throw error;
  }
};
...
const subscribeToNotifications = async (event) => {
  try {
    ...
    logger.error('Error subscribing to notifications:', { errorMessage: error.message, errorType: error.constructor.name });
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ 
...
const listSubscriptions = async (event) => {
  try {
    ...
    for (const topicKey of topics) {
      ...
      if (topicArn) {
        try {
          ...
          logger.error('Error listing subscriptions for ${topicKey}:', { errorMessage: error.message, errorType: error.constructor.name });
    ...
    logger.error('Error listing subscriptions:', { errorMessage: error.message, errorType: error.constructor.name });
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ 
