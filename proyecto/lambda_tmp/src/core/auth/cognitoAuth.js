...
const getUserFromCognito = (event) => {
  try {
    ...
    logger.error('Error extrayendo usuario de Cognito:', { errorMessage: error.message, errorType: error.constructor.name });
    throw error;
  }
};
  ...
  return (handler) => {
    return async (event) => {
      try {
        ...
        logger.error('Error en middleware de autorización:', { errorMessage: error.message, errorType: error.constructor.name });
        return {
          statusCode: 401,
          headers: {
            'Content-Type': 'application/json',
