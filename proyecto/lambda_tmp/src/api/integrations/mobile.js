...
const { success, badRequest } = require('../../shared/utils/responses');
const { resilienceManager } = require('../../shared/utils/resilienceManager');

const db = new DynamoDBManager();

const getMobileDashboard = withSecureAuth(async (event) => {
...
const getMobileSpaces = withSecureAuth(async (event) => {
...
const getMobileReservations = withSecureAuth(async (event) => {
    const user = event.user;
...
const createMobileReservation = withSecureAuth(async (event) => {
    const user = event.user;
...
const getSpaceTypes = withSecureAuth(async (event) => {
    return await resilienceManager.executeDatabase(
        async () => {
