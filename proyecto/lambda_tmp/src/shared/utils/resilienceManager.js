  ...
  async executeWithFullResilience(operation, configKey, context = {}) {
    ...
    try {
      ...
      if (config.enableMetrics) {
        logger.info('[RESILIENCE_FULL]  ${config.serviceName} [${bulkheadPool}]: Operación exitosa en ${responseTime}ms');
      }
      ...
      if (config.enableMetrics) {
        ...
        logger.error('[RESILIENCE_FULL] ❌ ${config.serviceName} [${errorType}]: ${error.message}');
      }
